...
def energy_function(image):
    ...
    out = np.absolute(G[0]) + np.absolute(G[1])
    ### END YOUR CODE
...
def compute_cost(image, energy, axis=1):
    ...
    for i in range(1, H):
        cand = np.array([cost[i - 1, :W], cost[i - 1, 1:W + 1], cost[i - 1, 2:W + 2]])
...
def enlarge_naive(image, size, axis=1, efunc=energy_function, cfunc=compute_cost, bfunc=backtrack_seam,
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost, dfunc=duplicate_seam, bfunc=backtrack_seam,
            rfunc=remove_seam):
